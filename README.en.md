...
- `anthropic`: keeps the standard Anthropic-compatible translation path, including plain-message `input_image` / `input_file` and `stop`
...
- `safety_identifier`
- `service_tier`
- `input_file.file_id` for all provider profiles
- `input_image.file_id` for all provider profiles
- `input_image.detail` values other than `auto` for all provider profiles
- plain-message `input_image` / `input_file` when `UPSTREAM_COMPAT_PROFILE=minimax`
